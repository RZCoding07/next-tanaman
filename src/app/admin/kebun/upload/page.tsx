...
// import { KebunType } from "types/kebun";
...
const UploadIplFile: React.FC = (): JSX.Element => {
    ...
    const [mappedData, setMappedData] = useState<KebunType[]>([]);
    ...
    const handleUploadKebun = async (mappedData, apiUrl, setIsLoadingUpload, setProgressValue, setIsUploadingDone) => {
        ...
        try {
            ...
            for (let i = 0; i < Math.ceil(mappedData.length / chunkSize); i++) {
                const res = await fetch(`${apiUrl}/kebun/upload`, {
                    method: "POST",
                    headers: {
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-end gap-3 mt-5">
                ...
                {!isLoadingUpload ? (
                    <button
                        onClick={() => handleUploadKebun(mappedData, apiUrl, setIsLoadingUpload, setProgressValue, setIsUploadingDone)}
                        disabled={
                            !fileName ||
                            isLoadingUpload}
