...
const KebunList = () => {
  if (isWindowAvailable()) document.title = "Kebun List";
  ...
  const currentItems = dataAllKebun
    .filter((kebun) => {
      if (searchInput === "") {
        return kebun;
      } else if (
              ...
              <tbody>
                {isLoading ? (
                  <tr className="border-b border-[#9FA284] border-opacity-20 hover:bg-gray-50">
                    <td colSpan={5} className="px-5 p-7">
                      <PageLoading />
                    </td>
                  </tr>
                ) : (
                    ...
                    {dataAllKebun?.length === 0 ? (
                      ...
                      <>
                        {currentItems?.map((kebun, index) => (
                          ...
                          >
                            <td className="px-6 py-7">{indexOfFirstItem + 1 + index}</td>
            ...
            <nav className="flex items-center justify-between p-8 dark:bg-navy-800 bg-gray-50" aria-label="Table navigation">
              <span className="text-sm font-normal text-gray-500">
                ...
                <span className="font-semibold text-gray-900 dark:text-white">
                  {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, dataAllKebun?.length)}
                </span>{" "}
                of{" "}
                <span className="font-semibold text-gray-900 dark:text-white">
                  {dataAllKebun?.length}
              ...
              <ul className="inline-flex items-center -space-x-px">
                <li>
                  <button
                    className={`block px-3 py-2 ml-0 leading-tight text-gray-500 border shadow-lg dark:border-navy-900 rounded-l-lg ${currentPage === 1 ? "cursor-not-allowed" : "hover:bg-gray-100 hover:text-gray-700"}`}
                ...
                {Array.from({ length: Math.ceil(dataAllKebun?.length / itemsPerPage) }, (_, i) => (
                  <li key={i}>
                    <button
                ...
                <li>
                  <button
                    className={`block px-3 py-2 leading-tight text-gray-500 border shadow-lg dark:border-navy-900 rounded-r-lg ${currentPage === Math.ceil(dataAllKebun?.length / itemsPerPage) ? "cursor-not-allowed" : "hover:bg-gray-100 hover:text-gray-700"}`}
                    onClick={() => paginate(currentPage + 1)}
                    disabled={currentPage === Math.ceil(dataAllKebun?.length / itemsPerPage)}
                  >
