...
const EditKaryawan =  async ({params}: {params:{id:string}} ) => {
    ...
    if (isWindowAvailable()) document.title = "Edit Karyawan";

    const getDataUser = async () => {
        const loginData = cookie.get("token");
        const tokenData: Tokens = JSON.parse(loginData || "{}");
    
        const res = await fetch(`${apiUrl}/karyawan/${params.id}`, {
          method: "GET",
          headers: {
            ...
            <div className="w-full p-5 mx-auto mt-10 mb-10 border shadow-xl lg:w-3/4 rounded-2xl dark:bg-navy-800 dark:border-navy-800">
                ...
                <form onSubmit={handleSubmit(handleEditKaryawan)}>
                    ...
                    <div className="flex justify-end mt-5">
                        <button
                            type="submit"
                            className="px-5 py-2 text-sm text-white bg-green-500 rounded-lg focus:outline-none hover:bg-orange-400 dark:bg-navy-600 dark:hover:bg-orange-400"
                        >
                            Edit Karyawan
                        </button>
                    </div>
